...
export class Dashboard implements OnInit {
  ...
  parseGroups() {
    if (!this.rawStats?.groups) return;
    const raw = this.rawStats.groups;
    const arr: GroupStat[] = Array.isArray(raw) ? raw : Object.values(raw);
    this.groups = arr.map((g: GroupStat) => ({ id: g.id, nombre: g.nombre }));
    this.rawStats.groups = arr;
  }
